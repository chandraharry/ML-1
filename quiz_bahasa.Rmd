...
```{r}
# your code here
aggregate(formula = average_montly_hours ~ division, data = turnover, FUN = mean) # cara 1
```
...
set.seed(100)
# your code here
index <- sample(nrow(turnover), # jumlah data
                nrow(turnover)*0.8) # ambil 80% utk data train
```
...
turnover.test <- turnover[-index,]

prop.table(table(turnover.train$left))
```
...
2. Berdasarkan proporsi di atas, dapatkah kita mengatakan bahwa distribusi kelas target seimbang? Mengapa kita perlu memastikan bahwa kelas target memiliki proporsi yang seimbang terutama pada data *train*?
  ...
  - [X] Iya. Distribusi kelas target perlu kita buat seimbang agar algoritma/model dapat mempelajari karakteristik setiap kelas dengan seimbang, ketika melakukan model fitting.
...
```{r}
model_logistic <- glm(formula = left ~ ., # suluruh prediktor
                    data = turnover.train, 
...
3. Regresi logistik adalah salah satu model yang bisa diinterpretasikan. Kita dapat menjelaskan bagaimana masing-masing variabel memberikan pengaruh pada hasil prediksi. Berdasarkan ringkasan model di atas, apa yang dapat diinterpretasikan dari koefisien `Work_accident1`?
  ...
  - [X] Karyawan yang memiliki kecelakaan kerja 0.21 kali lebih mungkin untuk mengundurkan diri dibandingkan karyawan yang tidak memiliki kecelakaan kerja.
...
#glimpse(turnover.train)
# variabel prediktor pada `train`
train_x <- turnover.train[,-10]
# variabel target pada `train`
train_y <- turnover.train[,-10]


# variabel prediktor pada `test`
test_x <- turnover.test[,10]
# variabel target pada `test`
test_y <- turnover.test[,10]
```
...
turnover_pred <- knn(train = train_x, # prediktor train
                 test = test_x, # prediktor test
                 cl = train_y, # target aktual train
                 k = 75)
```
...
```{r}
# your code here
round(sqrt(nrow(turnover.train)))
```
...
5. Isilah kerangka *code* di atas dan pilihlah *code* yang tepat di bawah ini untuk membangun model *kNN*!
  ...
  - [X] model_knn <- knn(train = train_x, test = test_x, cl = train_y, k = 75)
...
prob_value <- predict(object = model_logistic,
                    newdata = turnover.test,
                    type = "response")

head(prob_value)

#prob_value <-predict(object = model_logistic, # model
#        newdata = turnover.test,  # data baru
...
# your code here
library(caret)
confusionMatrix(data = turnover.test$left, 
                reference = turnover.test$left,
                positive = "1")
```
...
9. Sekarang, ingatlah apa yang sudah kita pelajari mengenai kelebihan masing-masing model. Manakah model yang lebih tepat digunakan bila kita bertujuan untuk mendapatkan model yang dapat diinterpretasikan?
  ...
  - [X] Regresi logistik, karena setiap koefisien variabel dapat diubah menjadi *odds ratio*
